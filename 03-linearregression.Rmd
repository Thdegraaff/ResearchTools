...
```{r, fig.cap='A scatterplog between miles per gallon and car weight.'}
plot(mpg~wt, mtcars)
```
...
```{r, fig.cap='A scatterplog between miles per gallon and car weight with regression line.'}
plot(mpg~wt, mtcars)
...
Unfortunately, creating tables of your own is a pain in the ... Luckily, within `R` (and by the way in programs as `Stata` as well) you can do this automatically! In `R` there are several packages one use. I prefer the package Stargazer which can be loaded as: 

```{r, message=FALSE}
library("stargazer")
```

And using this package we get the following outcome for our logarithmic specification:

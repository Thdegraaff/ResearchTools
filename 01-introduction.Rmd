...
- They are scriptable. This means they can "communicate" with other programs. Usually this means that these tools revolve around plain old `.txt` files and its related cousins (everything that can be opened and edited by an editor, such as source code, `.csv` and `.html` files). 
- There is a large community that uses these tools. This is very handy for complementary materials (books, packages, etc.) and solving questions. 
- They are multi-purpose, which means you can use them for various purposes. This is actually only a requirement of the data management/statistics tool you use. Simply said, you want a flexible tool that is capable of doing a variety of tasks.
...
4. it generates beautiful pictures, diagram, maps, and histograms (even pie diagrams for the masochistics amongst us);
...
In its present state, the book contains three chapters dealing with contents. The second chapter concerns the basic usage of `R`. The third deals with the fundamentals behind regression analysis and how to apply it in `R`. The fourth chapter lays out how to do network analysis in `R` (it is still in a very basic form).   